...
def process_input(process, input_data):
    ...
    main_output_path = process.configuration.get("OUTPUT_PATH")
    output_prefix = main_output_path + "/"  + database.output_summaries + "/" + database.name + "_" + lecture.title.lower()
...
def main():
    ...
    in_files = ["CC_ST3D_OUTPUT", "VIDEO_SEGMENTATION_OUTPUT"]
    process = ConsoleUIProcess.FromConfigPath(sys.argv[1], sys.argv[2:], in_files, "SUMMARY_KEYFRAMES_OUTPUT")
